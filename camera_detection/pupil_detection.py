...
class pupil_detection:
    ...
    def __init__(self,url,user_name,car_number):
    ...
    def enviro_detect(self):
        ...
        while (self.cap.isOpened()):
            if self.col_car.find_one({"_id": str(self.car_number)})['ifStart']:
                return "The owner of this car is inside:-)"
            else:
                ret, frame = self.cap.read()
                if ret == True:
                    eye = cv2.CascadeClassifier('haarcascade_eye.xml')
                    detected = eye.detectMultiScale(frame, 1.3, 3)
                    if detected != () and pic_count < 5:
                        ...
                        if serious:
                            ...
                            warning_info = {"user": self.car_number, "time": datetime.datetime.now(),
                                            "geolocation": [lat, lon],
    ...
    def detection(self):
        if self.col_account.find_one({"_id":self.user})['ifStart']:
            while (self.cap.isOpened()):
                ret, frame = self.cap.read()
                if ret == True:
                    # downsample
                    ...
                    if self.warning >= 40 and not self.has_record:
                        ...
                        warning_info = {"user":self.user,"time": datetime.datetime.now(), "geolocation":[lat, lon],
            ...
            print "The user doesn't start driving"
